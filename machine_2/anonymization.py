...
external_ip = os.getenv("EXTERNAL_IP", "35.242.194.252")
...
MACHINE_ID="machine-02-anonymization"
...
BUCKET_NAME=os.getenv("BUCKET_NAME","machine-02-bucket")
...
KMS_KEY_OWN_NAME = ("projects/machine-02-anonymization/locations/global/keyRings/machine-2-keyring/cryptoKeys/machine-2-key")
KMS_KEY_PREVIOUS_NAME = ("projects/machine-01-data-generation/locations/global/keyRings/machine-1-keyring/cryptoKeys/machine1-output-key")
...
def process_data(local_decrypted_path):
    ...
    if not os.path.exists(local_decrypted_path):
        raise FileNotFoundError(f"{local_decrypted_path} not found.")
    
    with open(local_decrypted_path, "r") as f:
        data=json.load(f)
        
    df = pd.DataFrame(data)
    logging.info("Before machine 2 computation: first 10 rows:\n%s", df.head(10).to_string())
    # remove Patient_ID (anonymization)
    if "Patient_ID" in df.columns:
        df_anonymized = df.drop(columns=["Patient_ID"])
    
    logging.info("After machine 2 computation: first 10 rows:\n%s", df_anonymized.head(10).to_string())
    df_anonymized=local_decrypted_path.replace("_decrypted.json", "_processed.json")
    df.to_json(df_anonymized, orient="records")
    logging.info(f"Processed dataset saved at {df_anonymized}")
    return df_anonymized

def encrypt_processed_file(local_processed_path):
    if TEST_MODE== "no_encryrption":
        logging.warning("WARNING: Skipping encryption due to TEST_MODE.")
        #copy
        local_unencrypted_output=local_processed_path.replace("_processed.json", f"_{FIXED_STEP_NUMBER}.bin")
        with open(local_processed_path, "rb") as src, open(local_unencrypted_output, "wb") as dst:
            dst.write(src.read())
        logging.info(f"File saved without encryption at {local_unencrypted_output}")
        return local_unencrypted_output
    logging.info("Encrypting processed output using hybrid encryption...")
    ...
    logging.info(f"Hybrid-encrypted file saved at {local_encrypted_output}")
...
def tamper_file(local_processed_path):
    logging.info("TAMPERING: Modifying file after hash calculation due to test mode.")
    with open(local_processed_path, "a") as f:
        f.write("\nTampered Data!")
...
def submit_audit_record(output_hash, public_url):
    try:
        logging.info("Submitting audit record...")
        ...
        if TEST_MODE=="wrong_previous_hash":
            logging.warning("WARNING:Submitting audit record with wrong previous hash due to test mode.")
            
            fake_previous_hash=os.urandom(32).hex()
            payload["fake_previous_hash"]=fake_previous_hash
...
def handle_process():
    try:
        logging.info("Machine 2 process started...")
        #0 register machine
        register_machine()
        ...
        #manipulate file if test mode
        if TEST_MODE=="tampered_output":
            tamper_file(local_processed_path)
        ...
        if TEST_MODE!="no_audit_submission":
            #10 new audit record
            submit_audit_record(output_hash, public_url)
        else:
            logging.warning("WARNING: Skipping audit submission due to test mode.")
            
        logging.info("Machine 2 process completed successfully")
        upload_log_to_gcs()
...
def register_machine():
    try:
        ...
        nonce = get_registration_nonce()["nonce"]
