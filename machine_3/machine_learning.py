...
external_ip = os.getenv("EXTERNAL_IP", "34.40.58.193")
...
MACHINE_ID="machine-03-data-analysis"
...
BUCKET_NAME=os.getenv("BUCKET_NAME","machine-03-bucket")
ML_NAME=os.getenv("ML_NAME","diabetes_model2.pkl")
...
KMS_KEY_OWN_NAME = "projects/machine-03-data-analysis/locations/global/keyRings/machine-3-keyring/cryptoKeys/machine-3-key"
KMS_KEY_PREVIOUS_NAME = "projects/machine-02-anonymization/locations/global/keyRings/machine-2-keyring/cryptoKeys/machine-2-key"



def get_ml_model_from_gcs():
    logging.info(f"Downloading ML model from GCS")
    
    bucket =storage_client.bucket(BUCKET_NAME)
    blob = bucket.blob(ML_NAME)
    local_model_path=f"/tmp/{ML_NAME}"
...
def process_data(local_decrypted_path, model):
    logging.info("Processing dataset...")
    with open(local_decrypted_path, "r") as f:
        data=json.load(f)
        
    df = pd.DataFrame(data)
    logging.info("Before machine 3 computation: first 10 rows:\n%s", df.head(10).to_string())
    ...
    if not all(col in X.columns for col in feature_columns):
        ...
        raise ValueError({"error": f"Missing columns for outcome: {missing_cols}"})
    ...
    logging.info("After machine 3 computation: first 10 rows:\n%s", df.head(10).to_string())
...
def handle_process():
    try:
        logging.info("Machine 3 process started...")
        ...
        #6 process data
        local_model_path=get_ml_model_from_gcs()
        ...
        #8 process data
        local_processed_path=process_data(local_decrypted_path, model)
    
        #9 calculate outputHash for audit trail record
        ...
        logging.info("Machine 3 process completed successfully")
        upload_log_to_gcs()
